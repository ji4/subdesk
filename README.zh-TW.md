...
SubDesk 把「**AI 挑錯字**」和「**人工核對**」放進同一個畫面，校字幕時不用再於 AI 對話視窗與剪輯軟體之間切來切去。

## 要解決的問題

影片剪輯軟體已經能用 AI 自動辨識字幕——但辨識結果還是會有錯字，得人工核對。這時你可能會把字幕再丟給另一個 AI（ChatGPT、Claude…）幫忙挑出可能的錯字。麻煩的是接下來：每一個被挑出的錯字，都要查時間戳、回剪輯軟體拉到那個時間點、聽完、再切回來改字。一個步驟一個視窗，一支影片切換幾十次。
...
┌─ 字幕列表（全部｜已修改）
│   00:01:12  ……………………
│   00:03:45  …錯字…（已修改）  ←─ 點擊＝跳轉播放
│   00:08:12  …錯字…（已修改）
└─

┌─ 校正後輸出　☑ 僅顯示已修改　☑ 顯示前後對比
│   #12 00:03:45 | 原文 | 修正後   ←─ 把 AI 的回覆貼到這裡
│   #27 00:08:12 | 原文 | 修正後
└─
```
